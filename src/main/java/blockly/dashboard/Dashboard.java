...
public static Var ObterDadosPedido() throws Exception {
 return new Callable<Var>() {
   ...
   public Var call() throws Exception {
    ...
    try {
         pedido = cronapi.database.Operations.query(Var.valueOf("farmacia.entity.Pedido"),Var.valueOf("select p.produto.nome, SUM(p.quantidade) from Pedido p  group by p.produto.id"));
...
public static Var ObterDadosVendas() throws Exception {
 return new Callable<Var>() {
   ...
   public Var call() throws Exception {
    ...
    dados = cronapi.list.Operations.newList();
    try {
         faturamento = cronapi.database.Operations.query(Var.valueOf("farmacia.entity.Venda"),Var.valueOf("select extract(day from v.data), \n        extract(month from v.data), \n        extract(year from v.data), SUM(v.valor) from Venda v \ngroup by extract(day from v.data), extract(month from v.data), extract(year from v.data)"));
        for (Iterator it_i = faturamento.iterator(); it_i.hasNext();) {
            i = Var.valueOf(it_i.next());
            cronapi.list.Operations.addLast(legendas,Var.valueOf(cronapi.list.Operations.get((cronapi.json.Operations.toJson(i)), Var.valueOf(1)).toString() + Var.valueOf("/").toString() + cronapi.list.Operations.get((cronapi.json.Operations.toJson(i)), Var.valueOf(2)).toString() + Var.valueOf("/").toString() + cronapi.list.Operations.get((cronapi.json.Operations.toJson(i)), Var.valueOf(3)).toString()));
            cronapi.list.Operations.addLast(dados,cronapi.list.Operations.getLast((cronapi.json.Operations.toJson(i))));
        } // end for
     } catch (Exception erro_exception) {
          erro = Var.valueOf(erro_exception);
